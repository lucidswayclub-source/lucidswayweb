(() => {
 ...
 scene.innerHTML=`<div class="deck-stage">${slides.map((a,i)=>`<button class="deck-card" data-index="${i}" aria-label="Show next memory: ${a.title}"><span class="deck-chrome"><span class="deck-lights" aria-hidden="true"><i></i><i></i><i></i></span><span>lucids / memories_0${i+1}</span><span>↗</span></span><span class="deck-image"><img src="${a.photos[0]}" alt="${a.title} Concert lights and crowd" draggable="false"><span class="deck-caption"><strong>${a.title}</strong></span></span><span class="deck-foot"><span>THE LUCID’S WAY</span><span>FRAME 0${i+1}</span></span></button>`).join('')}</div><div class="deck-controls"><a class="deck-open" href="/memories/${slides[0].id}/">Open the memory folder ↗</a><div><button class="deck-prev" aria-label="Previous memory">←</button><span class="deck-count" aria-live="polite">01 / 03</span><button class="deck-next" aria-label="Next memory">→</button></div></div>`;
